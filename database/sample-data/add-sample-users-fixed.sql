...
VALUES 
  ...
  ('2008', '佐藤二郎', 'player.sato.jiro2@example.com', 'じろう（さ〕', NULL, 'player', false, NOW(), NOW()),
  ('2009', '山田六郎', 'player.yamada.rokuro@example.com', 'ろくろう', NULL, 'player', false, NOW(), NOW()),
  
  -- 3年生
  ('2010', '高橋三郎', 'player.takahashi.saburo@example.com', 'さぶろう（た〕', NULL, 'player', false, NOW(), NOW()),
  ('2011', '渡辺七郎', 'player.watanabe.shichiro@example.com', 'しちろう', NULL, 'player', false, NOW(), NOW()),
  
  -- 2年生
  ('2012', '高橋さくら', 'player.takahashi.sakura@example.com', 'さくら', NULL, 'player', false, NOW(), NOW()),
  
  -- 1年生（学年バランスのため）
  ('2013', '中村八郎', 'player.nakamura.hachiro@example.com', 'はちろう', NULL, 'player', false, NOW(), NOW())
ON CONFLICT (pin) DO UPDATE SET
  name = EXCLUDED.name,
  email = EXCLUDED.email,
  hiragana_name = EXCLUDED.hiragana_name,
...
INSERT INTO users (pin, name, email, hiragana_name, line_id, role, is_admin, created_at, updated_at)
VALUES 
  ('3001', 'コーチA', 'coach.a@example.com', NULL, 'coach_line_001', 'coach', false, NOW(), NOW()),
  ('3002', 'コーチB', 'coach.b@example.com', NULL, 'coach_line_002', 'coach', false, NOW(), NOW()),
  ('3003', 'コーチC', 'coach.c@example.com', NULL, 'coach_line_003', 'coach', false, NOW(), NOW())
ON CONFLICT (pin) DO UPDATE SET
  name = EXCLUDED.name,
  email = EXCLUDED.email,
  hiragana_name = EXCLUDED.hiragana_name,
