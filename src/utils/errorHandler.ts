...
export class ErrorHandler {
  ...
  private renderMessage(errorInfo: ErrorInfo): void {
    ...
    // iPhoneライクなアニメーション - 上から下にスライドして少し止まる
    setTimeout(() => {
      messageElement.style.transform = 'translateY(0)';
      messageElement.style.opacity = '1';
    }, 10);

    // iPhoneライクな自動削除 - 少し待ってから上にスライドして消える
    setTimeout(() => {
  ...
  private getMessageClasses(type: string): string {
    const baseClasses = 'fixed px-6 py-2 shadow-lg z-50 flex items-center justify-center space-x-3 transition-all duration-500 error-message-toast text-base font-medium';
    
    switch (type) {
      case 'error':
        return `${baseClasses} bg-red-500 text-white transform -translate-y-full opacity-0`;
      case 'success':
        return `${baseClasses} bg-green-500 text-white transform -translate-y-full opacity-0`;
      case 'warning':
        return `${baseClasses} bg-yellow-500 text-white transform -translate-y-full opacity-0`;
      case 'info':
        return `${baseClasses} bg-blue-500 text-white transform -translate-y-full opacity-0`;
      default:
        return `${baseClasses} bg-gray-500 text-white transform -translate-y-full opacity-0`;
    }
  }
